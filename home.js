...
onAuthStateChanged(auth, async (user) => {
    ...
    try {
        ...
        if (myUsername) myUsername.textContent = username;
...
function loadUsers() {
    ...
    unsubscribeUsers = onSnapshot(collection(db, "users"), (snapshot) => {
        ...
        snapshot.forEach((userDoc) => {
            ...
            if (user.uid && currentUser && user.uid !== currentUser.uid) {
                allUsers.push(user);
            }
        });
...
function renderUsers() {
    ...
    const searchText = searchUsers ? searchUsers.value.toLowerCase().trim() : "";

    const sortedUsers = [...allUsers].sort((a, b) => {
        const chatA = conversations[a.uid];
        const chatB = conversations[b.uid];
        const timeA = getTimestamp(chatA?.lastMessageAt);
        const timeB = getTimestamp(chatB?.lastMessageAt);
        return timeB - timeA;
    });
...
function createUserElement(user) {
    ...
    const conversation = conversations[user.uid];

    let lastMessage = "Click to start chatting";
    let unreadCount = 0;

    if (conversation) {
        if (conversation.lastMessage) lastMessage = conversation.lastMessage;
        if (conversation.unreadFor && conversation.unreadFor[currentUser.uid]) {
            unreadCount = Number(conversation.unreadFor[currentUser.uid]) || 0;
        }
    }

    if (unreadCount > 0) userItem.classList.add("has-unread");
    if (selectedUser && selectedUser.uid === user.uid) userItem.classList.add("active");
...
async function openChat(user) {
    ...
    unsubscribeSelectedUser = onSnapshot(doc(db, "users", user.uid), (snap) => {
        if (snap.exists()) {
    ...
    try {
        ...
        // MARK ALL RECEIVED MESSAGES AS READ AND RESET UNREAD COUNT
...
// ========================================
// MARK MESSAGES AS READ  (CLEANED VERSION)
// ========================================

async function markMessagesAsRead(chatId) {
    if (!currentUser || !chatId) return;

    // 1. Optimistic local clear (badge disappears immediately)
    if (selectedUser && conversations[selectedUser.uid]) {
        const conv = conversations[selectedUser.uid];
    ...
    // 2. Clear the counter first
    try {
        await setDoc(chatRef, {
            [`unreadFor.${currentUser.uid}`]: 0
        }, { merge: true });
        console.log("Unread count set to 0 for", currentUser.uid);
    } catch (err) {
    ...
    try {
        ...
        const unreadQuery = query(messagesRef, where("receiverId", "==", currentUser.uid));
        ...
        console.error("Failed to mark individual messages as read:", err);
    }
...
function loadMessages() {
    ...
    unsubscribeMessages = onSnapshot(messagesQuery, async (snapshot) => {
        ...
        snapshot.forEach((messageDoc) => {
            ...
            if (data.receiverId === currentUser.uid && !data.readAt) {
...
function displayMessage(message) {
    ...
    bottomElement.className = "message-bottom";
    ...
    if (message.createdAt) {
        try {
            timeElement.textContent = message.createdAt.toDate().toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit"
            });
        } catch {
    ...
    if (isSent) {
        ...
        receipt.className = "message-status";
        ...
        bottomElement.appendChild(receipt);
    }

    messageElement.appendChild(textElement);
    messageElement.appendChild(bottomElement);
...
if (messageInput) {
    ...
    messageInput.addEventListener("input", updateKeyboardHeight);
...
if (logoutButton) {
    logoutButton.addEventListener("click", async () => {
        try {
            ...
            if (unsubscribeUsers) unsubscribeUsers();
            if (unsubscribeChats) unsubscribeChats();
            if (unsubscribeSelectedUser) unsubscribeSelectedUser();
